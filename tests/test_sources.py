"""Tests unitaires pour les fonctions utilitaires et les générateurs dlt des sources."""
...
class TestNormalizeTitle:
    """Vérifie que normalize_title produit des clés cohérentes entre MIT et Kaggle."""
...
class TestParseListStr:
    """Vérifie le parsing des listes sérialisées en chaîne (format Kaggle)."""
...
class TestParseNutrition:
    """Vérifie que _parse_nutrition extrait la première valeur (kcal/portion) du vecteur Kaggle."""
...
class TestLayer1Resource:
    """Vérifie la lecture et la transformation du fichier layer1 MIT."""
    ...
    def test_instructions_concatenated(self) -> None:
        """Les étapes doivent être jointes par ' | '."""
        with patch(f"{_CFG}.LAYER1_PATH", FIXTURES_DIR / "layer1.json"):
...
class TestLayer2Resource:
    """Vérifie l'extraction des images depuis layer2 et le filtrage des recettes sans image."""
    ...
    def test_only_recipes_with_images(self) -> None:
        """Les recettes sans image doivent être exclues du résultat."""
        with patch(f"{_CFG}.LAYER2_PATH", FIXTURES_DIR / "layer2+.json"):
...
class TestDetIngrsResource:
    """Vérifie le filtrage des ingrédients invalides dans det_ingrs."""

    def test_filters_invalid_ingredients(self) -> None:
        """Un ingrédient marqué valid=False ne doit pas figurer dans ingredients_validated."""
...
class TestNutritionResource:
    """Vérifie l'aplatissement du vecteur nutritionnel MIT."""
