"""Tests unitaires pour les expressions Spark de enrich.py."""
...
class TestNutriScoreCol:
    """Vérifie les seuils de _nutri_score_col, à maintenir en sync avec la doc du pipeline."""
...
class TestEnrich:
    """Vérifie les transformations de _enrich sur un schéma minimal issu de assemble()."""
...
class TestBuildIngredientsIndex:
    """Vérifie l'explosion et la normalisation des ingrédients pour l'index."""
...
class TestBuildNutritionDetail:
    """Vérifie le filtrage et la déduplication des lignes nutritionnelles."""
    ...
    def test_keeps_row_with_at_least_one_value(self, spark: SparkSession) -> None:
        """Une ligne entièrement nulle doit être exclue du résultat."""
        rows = [
            ("r1", 15.0, None, None, None, None),
            ("r2", None, None, None, None, None),
        ]
        ids = {
            r["recipe_id"]
            for r in build_nutrition_detail(
